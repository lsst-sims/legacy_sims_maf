...
def makeBundleList(dbFile, runName=None, benchmark='design'):
    ...
    for i, prop in enumerate(['All Props', 'WFD', 'DD']):
        for f in filters:
            ...
            if prop != 'DD':
                ...
                bundleList.append(bundle)
                # Calculate the median delivered FWHMeff.
                metric = metrics.MedianMetric(col='FWHMeff')
                plotDict={'normVal':benchmarkVals['FWHMeff'][f],
                          'xlabel':'Median FWHMeff/(Expected FWHMeff %.2f)'%(benchmarkVals['FWHMeff'][f]),
                          'cmap':cm.RdBu_r, 'colorMin':0.475, 'colorMax':1.525}
                displayDict={'group':seeinggroup, 'subgroup':subgroup, 'order':filtorder[f],
                             'caption':
                             'Median FWHMeff in filter %s divided by expected value (%.2f), %s.'
                             %(f, benchmarkVals['FWHMeff'][f], propCaption)}
                bundle = metricBundles.MetricBundle(metric, slicer, sqlconstraint, plotDict=plotDict,
                                                    displayDict=displayDict, runName=runName, metadata=metadata,
                                                    summaryMetrics=summaryStats)
                histMerge={'color':colors[f], 'label':'%s'%(f),
                           'xlabel':'FWHMeff/benchmark FWHMeff',
                           'binsize':.05, 'xMin':0.475, 'xMax':1.525,
                           'legendloc':'upper right'}
    ...
    for i, prop in enumerate(['All Props', 'WFD']):
        for f in filters:
            ...
            metric = metrics.CountMetric(col='FWHMeff', metricName='FWHMeff Histogram')
            histMerge={'legendloc':'upper right',
                       'color':colors[f], 'label':'%s'%f}
            displayDict={'group':seeinggroup, 'subgroup':subgroup, 'order':filtorder[f],
                         'caption':'Histogram of the seeing in %s band, %s.' %(f, propCaption)}
            slicer = slicers.OneDSlicer(sliceColName='FWHMeff', binsize=0.02)
            bundle = metricBundles.MetricBundle(metric, slicer, sqlconstraint, plotDict=plotDict,
                                                displayDict=displayDict, runName=runName, metadata=metadata)
    ...
    for i, propid in enumerate(props):
        ...
        for f in filters+['all']:
            ...
            cols = ['FWHMeff', 'filtSkyBrightness', 'airmass', 'fiveSigmaDepth', 'normairmass', 'dist2Moon']
