...
if __name__ == '__main__':
    ...
    restoreFile = glob.glob('gaiaStarDensity_%s_%s_nside_%i.npz' % (filterName, starNames, nside))
    ...
    for i in np.arange(indxMin, npix):
        ...
        if (i % checksize == 0) & (i != 0):
            np.savez('gaiaStarDensity_%s_%snside_%i.npz' % (filterName, starNames, nside),
    ...
    np.savez('gaiaStarDensity_%s_%snside_%i.npz' % (filterName, starNames, nside), starDensity=starDensity,
