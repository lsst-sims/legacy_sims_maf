...
class VisitGroupsMetric(ComplexMetric):
    ...
    def run(self, dataSlice):
        ...
        Count two visits which are within tmin of each other, but which have another visit within tmin/tmax interval, as one and a half (instead of two)."""
        ...
        for n in uniquenights:
            ...
            if len(timegood)>1:
                ...
                for i in range(len(timediff)-1):
                    ...
                    if timetooclose[i]:
                        ntooclose += 1
                        if ~timegood[i+1] and ~timetooclose[i+1]:
                            ntooclose += 1
    ...
    def reduceNVisitsInWindow(self, (visits, nights), minNVisits=None, window=None):
        ...
        maxnvisits = 0
        for n in nights:
            vw, nw = self._inWindow(visits, nights, n, window, minNVisits)
    ...
    def reduceNLunations(self, (visits, nights), minNVisits=None, window=None, minNNights=None):
        ...
        lunationLength = 30
        lunations = np.arange(nights[0], nights[-1:]+lunationLength/2.0, lunationLength)
        nLunations = 0
        for l in lunations:
            # Find visits within lunation.
            vl, nl = self._inLunation(visits, nights, l, lunationLength)
            for n in nl:
                # Find visits which are in groups within the lunation.
                vw, nw = self._inWindow(vl, nl, n, window, minNVisits)
                if len(nw) >= minNNights:
                    nLunations += 1
                    break
        return nLunations

    def reduceMaxSeqLunations(self, (visits, nights), minNVisits=None, window=None, minNNights=None):
