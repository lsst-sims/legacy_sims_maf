...
class TransientDetectMetric(BaseBetric):
    """
    ...
    """
    def __init__(self, mjdCol='expMJD', m5Col='fiveSigmaDepth', filterCol='filter', transDuration=10.,
        ...
        riseSlope = slope of the light curve before peak time (XXX -- should be negative since mags are backwards?)
        declineSlope = slope of the light curve after peak time
        ...
        self.mjdCol = expMJD
        self.m5Col = m5Col
        self.fitlerCol = filterCol
        super(TransientDetectMetric, self).__init__(col=[self.mjdCol, self.m5Col,self.filterCol], **kwargs)
    ...
    def run(self, dataSlice, slicePoint=None):
        ...
        nTransMax = self.surveyDuration/self.transDuration
        ...
        riseSec = np.where(time <= self.peakTime)
        lcMags[riseSec] += self.riseSlope*time[riseSec]
        declineSec = np.where(time > self.peakTime)
        lcMags[declineSec] += self.declineSlope*time[riseSec]

        detected = np.zeros(dataSlice.size, dtype=int)
        detected[np.where(lcMags < data[self.m5Col])] = 1
