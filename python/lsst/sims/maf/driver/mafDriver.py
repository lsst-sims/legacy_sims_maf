...
class MafDriver(object):
    ...
    def getData(self, constraint, colnames=[], stackersList=[]):
        ...
        # If there is a 'from' in the constraint, strip it out and pass that as the table to use
        if 'from' in constraint:
           splitConst = constraint.split(' ')
           tableName = [splitConst[i+1] for i,c in enumerate(splitConst) if splitConst[i] == 'from' ][0]
           splitConst.remove('from')
           splitConst.remove(tableName)

           constraint = ' '.join(splitConst)
           self.data = self.opsimdb.fetchMetricData(sqlconstraint=constraint,colnames=colnames,
                                                    distinctExpMJD=False, groupBy=None, tableName=tableName+'Table')
