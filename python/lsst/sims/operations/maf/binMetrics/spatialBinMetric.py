...
class SpatialBinMetric(BaseBinMetric):

    def setBin(self, binner):
        super(SpatialBinMetric, self).setBin(binner)
        # Check this grid is a spatial type.
        if self.binner.binnertype != 'SPATIAL':
            raise Exception('Binnertype for bin should be SPATIAL, not %s' %(self.binner.binnertype))
