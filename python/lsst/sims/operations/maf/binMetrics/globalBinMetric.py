...
class GlobalBinMetric(BaseBinMetric):
    ...
    def setBin(self, binner):
        super(GlobalBinMetric, self).setBin(binner)
        # Check this grid is a spatial type.
        if self.binner.binnertype != 'GLOBAL':
            raise Exception('Binnertype for binner should be GLOBAL, not %s' %(self.grid.gridtype))
        return
    ...
    def runBin(self, metricList, simData, 
        ...
        super(GlobalBinMetric, self).runBin(metricList, simData, simDataName=simDataName,
        ...
        for m in self.metrics:
            ...
            self.metricHistValues[m.name] = np.zeros((len(self.binner), histlen+1), dtype ='int')
            self.metricHistBins[m.name] = np.zeros((len(self.binner), histlen+1), dtype= 'float')
        # Run through all gridpoints and generate histograms 
        for i, g in enumerate(self.binner):
            idxs = self.binner.sliceSimData(g, simData[sliceCol])
            slicedata = simData[idxs]
            if len(idxs)==0:
                # No data at this gridpoint.
                for m in self.metrics:
                    self.metricHistValues[m.name][i] = self.binner.badval
                    self.metricHistBins[m.name][i] = self.binner.badval
            else:
                for m in self.metrics:
    ...
    def writeMetric(self, metricName, comment='', outfileRoot=None, outDir=None, 
        ...
        self.binner.writeMetricData(outfile, self.metricValues[metricName].filled(), 
    ...
    def plotMetric(self, metricName, 
        ...
        """Plot histogram for 'metricName' (global binMetric)."""
        ...
        try:
            ...
            raise ValueError('Metric %s does not have histogram data in binMetric.' %(metricName))
        ...
        fignum = None
        for i, g in enumerate(self.binner):
            fignum = self.binner.plotBinnedData(self.metricHistBins[metricName][i], 
    ...
    def plotComparisons(self, metricNameList, plotTitle=None, legendloc='upper left',
        ...
        for sim, meta, datacol, metric in zip(simDataNames, metadatas, datacolumns, metricNames): 
            ...
            for i, g in enumerate(self.binner):
                histfignum = self.binner.plotBinnedData(self.metricHistBins[metric][i], 
    ...
    def computeSummaryStatistics(self, metricName, summaryMetric=None):
        ...
        if summaryMetric == None:
            ...
            good = np.where(self.metricValues[metricName] != self.binner.badval)
