...
root.dbAddress ={'dbAddress':'sqlite:///../examples/opsim_small.sqlite'}
...
m1 = makeMetricConfig('CountMetric', params=['normairmass'], kwargs={'metricName':'normAir_hex'})
metricDict=makeDict(m1)
stacker = ColStackConfig()
stacker.name='NormAirmass'
stacker.kwargs_str = {'decCol':'hexdithdec'}
binner = makeBinnerConfig('OneDBinner', kwargs={"sliceDataColName":'normairmass'},
                          metricDict=metricDict, constraints=constraints, stackCols = makeDict(stacker))
binList.append(binner)

m1 = makeMetricConfig('CountMetric', params=['normairmass'])
metricDict=makeDict(m1)
binner = makeBinnerConfig('OneDBinner', kwargs={"sliceDataColName":'normairmass'},
                          metricDict=metricDict, constraints=constraints)
binList.append(binner)
