...
class TestBaseBinMetric(unittest.TestCase):
    def setUp(self):
    ...
    def testSetBinner(self):
        ...
        binner = binners.HealpixBinner(nside=16, verbose=False)
        self.assertFalse(self.testbbm.setBinner(binner, override=False))
        # Unless you really wanted to..
        self.assertTrue(self.testbbm.setBinner(binner, override=True))

    def testSetMetrics(self):
        """Test setting metrics."""
        m1 = metrics.MeanMetric('testdata', plotParams={'units':'meanunits'})
        m2 = metrics.CountMetric('testdata', plotParams={'units':'countunits',
                                                         'title':'count_title'})
        self.testbbm.setMetrics([m1, m2])
        ...
        self.assertEqual(self.testbbm.metricObjs.values(), [m1, m2])
...
if __name__ == '__main__':
    suite = unittest.TestLoader().loadTestsFromTestCase(TestBaseBinMetric)
