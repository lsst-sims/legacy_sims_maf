...
class TestSimpleMetrics(unittest.TestCase):
    ...
    def testMaxMetric(self):
        ...
        self.assertEqual(testmetric.run(self.dv, None), self.dv['testdata'].max())

    def testMinMetric(self):
        """Test min metric."""
        testmetric = metrics.MinMetric('testdata')
        self.assertEqual(testmetric.run(self.dv, None), self.dv['testdata'].min())

    def testMeanMetric(self):
        """Test mean metric."""
        testmetric = metrics.MeanMetric('testdata')
        self.assertEqual(testmetric.run(self.dv, None), self.dv['testdata'].mean())

    def testMedianMetric(self):
        """Test median metric."""
        testmetric = metrics.MedianMetric('testdata')
        self.assertEqual(testmetric.run(self.dv, None), np.median(self.dv['testdata']))

    def testFullRangeMetric(self):
        """Test full range metric."""
        testmetric = metrics.FullRangeMetric('testdata')
        self.assertEqual(testmetric.run(self.dv, None), self.dv['testdata'].max()-self.dv['testdata'].min())

    def testCoaddm5Metric(self):
        """Test coaddm5 metric."""
        testmetric = metrics.Coaddm5Metric(m5col='testdata')
        self.assertEqual(testmetric.run(self.dv, None), 1.25 * np.log10(np.sum(10.**(.8*self.dv['testdata']))))

    def testRmsMetric(self):
        """Test rms metric."""
        testmetric = metrics.RmsMetric('testdata')
        self.assertEqual(testmetric.run(self.dv, None), np.std(self.dv['testdata']))

    def testSumMetric(self):
        """Test Sum metric."""
        testmetric = metrics.SumMetric('testdata')
        self.assertEqual(testmetric.run(self.dv, None), self.dv['testdata'].sum())

    def testCountMetric(self):
        """Test count metric."""
        testmetric = metrics.CountMetric('testdata')
        self.assertEqual(testmetric.run(self.dv, None), np.size(self.dv['testdata']))
    ...
    def testRobustRmsMetric(self):
        ...
        self.assertEqual(testmetric.run(self.dv, None), rms_approx)

    def testIdentityMetric(self):
        """Test identity metric."""
        testmetric = metrics.IdentityMetric('testdata')
        self.assertEqual(testmetric.run(self.dv[0], None), self.dv[0]['testdata'])
        np.testing.assert_equal(testmetric.run(self.dv, None), self.dv['testdata'])

    def testFracAboveMetric(self):
        cutoff = 5.1
        testmetric=metrics.FracAboveMetric('testdata', cutoff = cutoff)
        self.assertEqual(testmetric.run(self.dv, None), np.size(np.where(self.dv['testdata'] >= cutoff)[0])/float(np.size(self.dv)))
    ...
    def testVisitGroups(self):
        ...
        metricval = testmetric.run(testdata, None)
